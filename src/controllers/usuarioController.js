...
//Seleccionar usuario admin y cliente
const seleccionar_usuario = async function(req, res){
    const Usuario = req.body.Usuario;
    const Clave = req.body.Clave;
    const sql1 = `SELECT Clave FROM usuarios WHERE Usuario = ${connection.escape(Usuario)}`;
    ...
    if(isEmptyObject(reg1)){
        ...
        if(esLaClave)
        {
            const sql = `SELECT * FROM usuarios WHERE Usuario = ${connection.escape(Usuario)} AND Clave = ${connection.escape(clave_hash)}`;
